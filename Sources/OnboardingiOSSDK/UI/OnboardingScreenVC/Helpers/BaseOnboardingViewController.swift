        ...
        DispatchQueue.main.async {
            if let media = self.media {
                if let percent = media.styles.heightPercentage {
                    self.mediaContainerViewHeightConstraint.constant = self.view.bounds.height * (percent / 100)
                } else {
                    let height = self.view.bounds.height - self.collectionView.contentSize.height
                    self.mediaContainerViewHeightConstraint.constant = height < 100 ? 100 : height
                }
            } else {
